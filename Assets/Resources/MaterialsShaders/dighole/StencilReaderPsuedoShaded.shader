Shader "Custom/StencilHole_BottomFakeLight" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
            ...
            fixed lightIntensity = (dot(fixed3(_LightDir.x, _LightDir.y, _LightDir.z), IN.worldNormal));

            //Use the toon lighting formula
            if (lightIntensity < _BrightnessCutoffA) {
                lightIntensity = 0;
            } else if (lightIntensity < _BrightnessCutoffB) {
                lightIntensity = _BrightnessMid;
            ...
            o.Emission = lightIntensity * _LightColor;
		}
		ENDCG
	}
	FallBack "Standard"
