...
public class ProceduralGrass : MonoBehaviour
{
    ...
    public float windOmega = 1f;
    [Tooltip("How fast wind changes over X")]
    public float windXFactor = 1f;

    [Tooltip("X grass meshes per terrain triangle. 0 results in nothing")]
    ...
    private GraphicsBuffer grassNormalBuffer;
    ...
    private void Start()
    {
        ...
        //MainManager.ListPrint(grassVertices);
        ...
        //MainManager.ListPrint(grassTriangles);
        ...
        grassMesh.GetNormals(normalSet);
        Vector3[] grassNormals = normalSet.ToArray();
        grassNormalBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, grassNormals.Length, sizeof(float) * 3);
        grassNormalBuffer.SetData(grassNormals);
        ...
        transformMatrixBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, terrainTriangleCount * density, sizeof(float) * 16);
        ...
        properties.SetBuffer("_Normals", grassNormalBuffer);
    ...
    private void RunComputeShader()
    {
        ...
        computeShader.SetInt("_Density", density);


        computeShader.SetFloat("_Wind", wind);
        computeShader.SetFloat("_WindOmega", windOmega);
        computeShader.SetFloat("_WindXFactor", windXFactor);

        // Run the compute shader's kernel function.
        computeShader.GetKernelThreadGroupSizes(kernel, out threadGroupSize, out _, out _);
        //int threadGroups = Mathf.CeilToInt((density * terrainTriangleCount) / (0.0f + threadGroupSize));
        threadCount = Mathf.CeilToInt((density * terrainTriangleCount) / (0.0f + threadGroupSize));
        DispatchInternal();
