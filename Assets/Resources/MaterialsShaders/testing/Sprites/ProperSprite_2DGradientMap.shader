Shader "Custom/ProperSprite_2DGradientMap" {
	Properties{
		[PerRendererData]_MainTex("Sprite Texture", 2D) = "white" {}
		_Cutoff("Shadow alpha cutoff", Range(0,1)) = 0.5
		_BlackColor("Black Color", Color) = (0,0,1,1)
		_WhiteColor("White Color", Color) = (0,0,1,1)
		_MaxSatColor("MaxSat Color", Color) = (0,0,1,1)
		_Leak("Leak", Range(0,1)) = 0.5
		_BacklightBorder("Backlight Border", float) = 0.5

		_Hue("Target Hue", Color) = (0,0,1,1)
		_SatPeak("Saturation Peak", Float) = 0.5
		_SatMult("Saturation Multiplier", Float) = 0.0
	...
	SubShader {
		...
		*/
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			float realHue = RGBToHSV(_Hue).x;

			float3 color = RGBToHSV(col.rgb);

			//set hue, cap saturation and value
			float diff = color.x - realHue;
			if (diff > 0.5) {
				//color.z = 0.1;
				diff = 1 - diff;
			}
			...
			float newSat = _SatPeak - abs(diff);
			if (newSat < 0) {
				newSat = 0;
			}
			//newSat *= 1 / SatPeak;
			newSat *= _SatMult;
			color.x = realHue;
			color.y *= (newSat/_SatPeak);


			fixed4 gradientColor = lerp(_BlackColor,_WhiteColor,color.z);

			float a = color.y;

			gradientColor = lerp(gradientColor, _MaxSatColor, color.y * color.z);

			fixed4 newcol = lerp(gradientColor, col, _Leak);
			newcol.a = col.a;
