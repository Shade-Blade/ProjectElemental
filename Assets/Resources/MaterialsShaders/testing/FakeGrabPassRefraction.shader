...
Shader "Custom/GrabPassCrystal" {
    Properties {        
        _Color ("Color", Color) = (1,1,1,1)
        _FresnelColor ("Fresnel Color", Color) = (1,1,1,1)
        _GlowFresnelPower ("Fresnel Glow Power", float) = 0
        _FresnelPower ("Fresnel Distortion Power", float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                float distFresnel = pow(fresnel, _GlowFresnelPower);
                ...
                */
                fixed4 bgcolor = tex2Dproj(_BackgroundTexture, checkPos);


                fixed4 c = lerp(_Color, _FresnelColor, 1 - distFresnel);
                fixed4 realcrystal = lerp(c, bgcolor, 1 - c.a);

                return fixed4(realcrystal.xyz, 1);
            }
            ENDCG
        }
