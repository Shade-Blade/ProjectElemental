Shader "Custom/UnlitDynamicDecalCircle"{
		...
		Pass{
			...
			fixed4 frag(v2f i) : SV_TARGET{
			  ...
			  //read the texture color at the uv coordinate
				...
				fixed4 col = fixed4(0,0,0,0);//tex2D(_MainTex, uv);

				if (length(uv - float2(0.5,0.5)) < _Radius) {
					col = fixed4(1,1,1,1);
				} else {
					col = fixed4(1,1,1, 1 - saturate(50 * (length(uv - float2(0.5,0.5)) - _Radius)));
				}

				//make the edge less hard slightly
