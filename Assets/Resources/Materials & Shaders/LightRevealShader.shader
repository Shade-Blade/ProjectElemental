...
Shader "Custom/LightRevealMaterial"
{
	Properties
	{
		_MainTex("Base (RGB)", 2D) = "white" {}
		_NoiseTex("Noise (color channels are independent)", 2D) = "white" {}
		...
		_WeakLightAlpha("Weak Light Alpha", Float) = 0.2
		...
		SubShader
		{
			...
			void surf(Input IN, inout SurfaceOutput o)
			{
				...
				weakdist += _DistanceMove * sin(3 * _Time.y);
				dist = lightValue + weakdist;
				dist = clamp(dist,0,1);
				...
				if (dist > _Cutoff) {
					if (weakdist < _WeakCutoff) {
						float modifiedDistW = weakdist / _WeakCutoff;
						modifiedDistW = (1) - modifiedDistW * (1 - _EdgeLight);
						o.Emission = 0.3 * _EmissionMult * pow(modifiedDistW,2);
						o.Alpha = _WeakLightAlpha;	
					} else {
						o.Alpha = _OuterAlpha;	
					}
				} else {
