Shader "Custom/SimpleWaterTexturedFixed"
{
    Properties
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i, UNITY_VPOS_TYPE vpos : VPOS) : SV_Target
            {
                ...
                if (diff < _FoamLength) {
                    ...
                    float2 wpA = float2(i.worldPos.x, i.worldPos.z);
                    float2 wpB = float2(worldPos.x, worldPos.z);
                    //float2 wpAb = lerp(wpA,wpB, 0.5);

                    //there is no "top surface"
                    //fixed4 frontColor = tex2D(_MainTex, wpA * _TextureScale);
                    //fixed4 frontColorB = tex2D(_MainTex, wpAb * _TextureScale);
                    fixed4 topColor = tex2D(_MainTex, wpB * _TextureScale);

                    //frontColor = lerp(frontColor, frontColorB, 0.5);

                    fixed4 newColor = topColor;


                    glowColor = lerp(newColor, _GlowColor, intersect);
                    glowColor.a = intersect;
                }
