Shader "VertexDistortion/Waves" {
	...
	SubShader {
		...
		float3 Warp(float3 v)
		{
			...
			float sinwave = sin(_Time.x * 4 + dist);
			dist = _MaxCurvature * saturate(dist / _MaxCurvature);

			wpos.y -= dist * sinwave * _CurvatureStrength;
