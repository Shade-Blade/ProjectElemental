Shader "Custom/ProperSprite_MultiPowerOverlay" {
	Properties{
		...
		_OverlayColorA("Overlay Color A", Color) = (0,0,0,0)
		_OverlayColorB("Overlay Color B", Color) = (0,0,0,0)
		_OverlayColorC("Overlay Color C", Color) = (0,0,0,0)
		_OverlayColorD("Overlay Color D", Color) = (0,0,0,0)
		_OverlayColorE("Overlay Color E", Color) = (0,0,0,0)
		_OverlayColorF("Overlay Color F", Color) = (0,0,0,0)
		_OverlayColors("Overlay Color Count", int) = 1
		...
		_XOffsetB("X Offset B", Range(0,1)) = 1	//the scrolling thing
	...
	SubShader {
		...
		*/
		...
		fixed4 _OverlayColorA;
		fixed4 _OverlayColorB;
		fixed4 _OverlayColorC;
		fixed4 _OverlayColorD;
		fixed4 _OverlayColorE;
		fixed4 _OverlayColorF;

		fixed4 OverlayColorArray[6];
		...
		void surf(Input IN, inout SurfaceOutput o) {
			OverlayColorArray[0] = _OverlayColorA;
			OverlayColorArray[1] = _OverlayColorB;
			OverlayColorArray[2] = _OverlayColorC;
			OverlayColorArray[3] = _OverlayColorD;
			OverlayColorArray[4] = _OverlayColorE;
			OverlayColorArray[5] = _OverlayColorF;
			...
			if (a.a != 0) {
				float numColorMult = (0.75 + _OverlayColors / 4);
				float timeFactor = _Time.x * _TimeScale * numColorMult;
				...
				float xoffset = 2 * abs(0.5 - frac(buv.x));

				float indexF = frac((xoffset + buv.y) / _OverlayColors);
				if (buv.y < 0) {
					indexF = 1 - indexF;
				}

				int index = int(indexF * _OverlayColors);

				fixed4 trueOverlayColor = OverlayColorArray[index];

				buv.x = buv.x + _XOffsetB;

				b = tex2D(_OverlayTex, buv) * trueOverlayColor;
