	...
	Properties
	{
		...
		SubShader
		{
			...
			void surf(Input IN, inout SurfaceOutput o)
			{
				...
				aetherValue = 1 - 4 * -_PlayerAether; //4 * _PlayerAether;
				aetherValue = clamp(aetherValue, 0, 1);					

				dist = aetherValue + (dx * dx + dy * dy + dz * dz) * (1 / _Radius);
				...
				float2 newUVb = float2(IN.worldPos.z, IN.worldPos.y);
