...
Shader "Custom/ProperSpriteBiGradientPrismatic" {
	Properties{
		...
		[HDR]
		_WhiteColor("Black Color", Color) = (0,0,1,1)
	...
	SubShader {
		...
		*/
		...
		float3 GetVoidColor(Input IN) {
			...
			fixed4 colR = tex2D(_ScreenTex, texScrollR) + 0.1;
			fixed4 colG = tex2D(_ScreenTex, texScrollG) + 0.1;
			fixed4 colB = tex2D(_ScreenTex, texScrollB) + 0.1;

            float3 vec = float3(colR.r, colG.g, colB.b);
            vec = normalize(vec);
            vec = float3(1 - vec.r, 1 - vec.g, 1 - vec.b);
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			fixed lerpCoeff = 1 - RGBToHSV(c.rgb).z;
			...
			if (lerpCoeff > _Midpoint) {
				...
				gradColor = lerp(half4(GetVoidColor(IN),1),_GrayColor,(lerpCoeff)/(_Midpoint));
			}
