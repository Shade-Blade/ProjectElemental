...
Shader "Custom/ProperSpriteBuffDebuffLuna" {
	Properties{
		...
		_RibbonCutoffA("Ribbon Cutoff A", float) = 0.1
		_RibbonCutoffB("Ribbon Cutoff B", float) = 0.4
		_WeaponCutoffA("Weapon Cutoff A", float) = 0.1
		_WeaponCutoffB("Weapon Cutoff B", float) = 0.4
		_WeaponCutoffC("Weapon Cutoff C", float) = 0.6
		...
		_OverlayTexB("Overlay Texture B", 2D) = "white" {}
		...
		_XOffsetB("X Offset B", Range(0,1)) = 1	//cyclic so values outside 0,1 are equivalent to something in that range		
	...
	SubShader {
		...
		*/
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			if (c.a > 0.01 && rbm > 0.08 && rdelta < 0.08 && c.g < 0.08) {
			...
			if (c.a != 0) {
				...
				cuv.x = cuv.x + _XOffsetB;
				buv.y = buv.y - timeFactor;
				cuv.y = cuv.y + timeFactor;
				...
				float cc = max(1, 3 - _OverlayColorB.x - _OverlayColorB.y - _OverlayColorB.z);
				b = (tex2D(_OverlayTex, buv) * _OverlayColor * bc + tex2D(_OverlayTexB, cuv) * _OverlayColorB * cc) * 0.8;
