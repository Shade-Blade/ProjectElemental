Shader "Hidden/WorldPositionScan"
{
    Properties
    {
        _MainTex ("-", 2D) = ""{}
        _LineStart("Start Distance", float) = 1
    ...
    fixed4 frag (v2f_img i) : SV_Target
    {
        ...
        float _FogStartC = _LineStart - _LineThickness;
        float _FogEndC = _LineStart;

        float fogValue = (distance - _FogStartC)/_LineThickness;
        if (fogValue > 1) {
            fogValue = 0;
        }
        ...
        fogCol.a = 1; //lerp(0, 1, fogValue);

        float _FogIntensityC = _FogColor.a * fogValue;

        return lerp(orCol, fogCol, _FogIntensityC);
    }
