Shader "CameraEffect/VignetteMaskedPrismatic" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            half4 fp(v2f i) : SV_Target {
                ...
                //note: arbitrary offsets remove that point where they all coincide (Very noticeable and bad)
                half4 mask = _MaskTex.Sample(point_repeat_sampler, float2(rx + xscr, ry + yscr + 0.1));
                half4 maskB = _MaskTex.Sample(point_repeat_sampler, float2(rx - xscr, ry + yscr + 0.3));
                half4 maskC = _MaskTex.Sample(point_repeat_sampler, float2(rx, ry + 2 * yscr));

                mask = half4(mask.r, maskB.g, maskC.b, 1);

                half z = x + y + _Offset;
                half zrA = z + _MaskPower * mask.r;
                half zgA = z + _MaskPower * mask.g;
                half zbA = z + _MaskPower * mask.b;
                ...
                output = half4(lerp(col.r, lerp(_Color.r, _ColorB.r, lerpCoeff), clamp(zr,1,2) - 1), lerp(col.g, lerp(_Color.g, _ColorB.g, lerpCoeff), clamp(zg,1,2) - 1), lerp(col.b, lerp(_Color.b, _ColorB.b, lerpCoeff), clamp(zb,1,2) - 1), 1);
