...
public class SignScript_PitFinalFloorSign : SignScript, ITextSpeaker
{
    ...
    public override IEnumerator SignCutscene()
    {
        ...
        testTextFile[0][0] += "<next>Total Playtime: " + MainManager.ParseTime((float)MainManager.Instance.playTime) + "<line>Time in front: " + (wilexPDE != null ? "<color,red>" + MainManager.ParseTime(wilexPDE.timeInFront) + "</color>" + (lunaPDE != null ? "/" : "") : "") + (lunaPDE != null ? "<color,green>" + MainManager.ParseTime(lunaPDE.timeInFront) + "</color>" : "");
