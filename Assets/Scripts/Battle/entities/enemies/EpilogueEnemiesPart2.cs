...
{
    public bool didSummon;
    int counterCount;
    public override void Initialize()
    {
        moveset = new List<Move> { gameObject.AddComponent<BM_Starfish_FeebleWave>(), gameObject.AddComponent<BM_Starfish_FatigueFog>(), gameObject.AddComponent<BM_Starfish_LeafStorm>(), gameObject.AddComponent<BM_Starfish_CounterBifurcate>()};
    ...
    public override bool ReactToEvent(BattleEntity target, BattleHelper.Event e, int previousReactions)
    {
        ...
        {
            if (!didSummon && BattleControl.Instance.FindUnoccupiedID(false, true) < 4)
            {
                counterCount++;
...
public class BM_Starfish_CounterBifurcate : EnemyMove
{
    public override MoveIndex GetMoveIndex() => MoveIndex.Starfish_CounterBifurcate;
    ...
    {
        BE_Starfish st = (BE_Starfish)caller;
        ...
        BE_Starfish newStarfish = (BE_Starfish)(BattleControl.Instance.SummonEntity(BattleHelper.EntityID.Starfish));
        ...
        if (newStarfish != null)
        {
            newStarfish.didSummon = true;
            newStarfish.hp = caller.hp;
            newStarfish.statMultiplier = st.statMultiplier;
            foreach (Effect e in caller.effects)
            {
                newStarfish.ReceiveEffectForce(e);
            }
            if (BattleControl.Instance.GetCurseLevel() > 0)
            {
                st.ReceiveEffect(new Effect(Effect.EffectType.QuantumShield, 1, 3));
                newStarfish.ReceiveEffect(new Effect(Effect.EffectType.QuantumShield, 1, 3));
            }

            newStarfish.transform.position = caller.transform.position + Vector3.forward * -0.05f;
            yield return StartCoroutine(newStarfish.JumpHeavy(newStarfish.homePos, 0.6f, 0.2f, 1));
            yield return new WaitForSeconds(0.1f);
            newStarfish.SetIdleAnimation(true);
            newStarfish.StartIdle();
        }
...
public class BE_DrainBud : BattleEntity
{
    ...
    public override void Initialize()
    {
        moveset = new List<Move> { gameObject.AddComponent<BM_DrainBud_PowerDrain>(), gameObject.AddComponent<BM_DrainBud_DrainBloom>(), gameObject.AddComponent<BM_DrainBud_Hard_MorbidBloomBoost>() };
...
public class BM_DrainBud_DrainBloom : EnemyMove
{
    public override MoveIndex GetMoveIndex() => MoveIndex.DrainBud_DrainBloom;
...
public class BM_DrainBud_Hard_MorbidBloomBoost : EnemyMove
{
    public override MoveIndex GetMoveIndex() => MoveIndex.DrainBud_Hard_MorbidBloomBoost;
