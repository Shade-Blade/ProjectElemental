...
public class LM_Brace : LunaMove
{
    ...
    public override int GetBaseCost(int level = 1)
    {
        ...
        return BaseCostCalculation(1, level, 3, 2);
    }
    public override BaseBattleMenu.BaseMenuName GetMoveType() => BaseBattleMenu.BaseMenuName.Jump;
    ...
    */
    ...
    public override IEnumerator Execute(BattleEntity caller, int level = 1)
    {
        ...
        IEnumerator MoveTracked()
        {
            ...
            //yield return caller.MoveEasing(target, caller.entitySpeed * 2f);
            move = false;
        }
                    ...
                    if (caller.GetAttackHit(targets[i], 0))
                    {
                        ...
                        } else
                        {
                            ...
                            switch (level)
                            {
                                case 1:
                                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Spin);
                                    caller.DealDamage(targets[i], Mathf.CeilToInt(sd / 2f) - 1, BattleHelper.DamageType.Normal, propertyBlock, BattleHelper.ContactLevel.Contact);
                                    break;
                                case 2:
                                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Spin);
                                    caller.DealDamage(targets[i], Mathf.CeilToInt(sd / 2f) - 0, BattleHelper.DamageType.Normal, propertyBlock, BattleHelper.ContactLevel.Infinite);
                                    break;
                                default:
                                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Spin);
                                    caller.DealDamage(targets[i], Mathf.CeilToInt(sd / 2f) + 2 * level - 4, BattleHelper.DamageType.Normal, propertyBlock, BattleHelper.ContactLevel.Infinite);
                                    break;
                            }
                        }
                    } else
...
public class LM_FlipKick : LunaMove
{
    ...
    public override IEnumerator Execute(BattleEntity caller, int level = 1)
    {
        ...
        if (caller.curTarget != null)
        {
            ...
            StartCoroutine(SpinSetAnimation());
            yield return StartCoroutine(caller.JumpHeavy(tpos, 1.5f, 0.5f, -0.25f, null, null, false));
    ...
    */
    ...
    public override IEnumerator Execute(BattleEntity caller, int level = 1)
    {
        ...
        if (caller.curTarget != null)
        {
            ...
            yield return StartCoroutine(caller.MoveEasing(bpos, 16, (e) => MainManager.EasingOut(e), false, false));
            ...
            if (GetOutcome(caller, sd))
            {
                if (result)
                {
                    if (level > 1)
                    {
                        ...
                        yield return StartCoroutine(caller.JumpHeavy(tpos2, 3f, 0.65f, 0.15f, true, true, MainManager.Sound.None));
                ...
                else
                {
                    ...
                    yield return StartCoroutine(caller.JumpHeavy(spos, 0.5f, 0.5f, 0.15f, true, true, MainManager.Sound.None));
                    yield return StartCoroutine(caller.MoveEasing(caller.homePos, (e) => MainManager.EasingOutIn(e)));
                }
            }
            else
            {
                ...
                //extrapolate the move curve
                //yield return StartCoroutine(caller.ExtrapolateJumpHeavy(spos, tpos, 2, 0.5f, -0.25f));
                yield return StartCoroutine(caller.JumpHeavy(spos, 2, 0.5f, 0.15f, true, true, MainManager.Sound.None));
                yield return StartCoroutine(caller.MoveEasing(caller.homePos, (e) => MainManager.EasingOutIn(e)));
            }
        }
        else
    ...
    {
        ulong propertyBlock = (ulong)BattleHelper.DamageProperties.AC_Success;
        caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Launch);
...
public class LM_ElementalStomp : LunaMove
{
    ...
    public override string GetHighlightText(BattleEntity caller, BattleEntity target, int level = 1)
    {
        ...
        for (int i = 1; i < 64; i++)
        {
            ...
            int temp = caller.DealDamageMultihitCalculation(target, sd + 2, (BattleHelper.DamageType)i, (ulong)BattleHelper.DamageProperties.AC_Success, 1, BattleHelper.MultihitReductionFormula.ReduceThreeFourths);
            ...
            if (GetOutcome(caller, other, sd))
            {
                ...
                if (result)
                {
                    ...
                    else
                    {
                        ...
                        yield return StartCoroutine(other.MoveEasing(other.homePos, (e) => MainManager.EasingOutIn(e)));
                    }
                ...
                else
                {
                    //Fly off into space
                    //extrapolate the move curve
                    //yield return StartCoroutine(other.MoveEasing(tpos2, 15));
...
public class LM_DoubleEgg : LunaMove
{
    ...
    public override TargetArea GetBaseTarget() => new TargetArea(TargetArea.TargetAreaType.Self, false);
    //public override float GetBasePower() => 0.5f;
    public override int GetBaseCost(int level = 1) => BaseCostCalculation(1, level, 3);
    public override BaseBattleMenu.BaseMenuName GetMoveType() => BaseBattleMenu.BaseMenuName.Jump;
...
public class LM_LeafySmash : LM_Smash
{
    public LM_LeafySmash()
    {
    }
    ...
    */
    ...
    public override void DealDamage(BattleEntity caller, int sd, bool result)
    {
        if (result)
        {
            ...
            switch (level)
            {
                case 1:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
                    caller.DealDamage(caller.curTarget, sd + 1, BattleHelper.DamageType.Earth, propertyBlock, BattleHelper.ContactLevel.Weapon);
                    break;
                case 2:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
                ...
                case 3:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
                    caller.DealDamage(caller.curTarget, sd + 7, BattleHelper.DamageType.Earth, propertyBlock, BattleHelper.ContactLevel.Weapon);
                    break;
                default:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
        ...
        else
        {
            switch (level)
            {
                case 1:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
                    caller.DealDamage(caller.curTarget, Mathf.CeilToInt(sd / 2f) + 1, BattleHelper.DamageType.Earth, 0, BattleHelper.ContactLevel.Weapon);
                    break;
                case 2:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
                ...
                case 3:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
                    caller.DealDamage(caller.curTarget, Mathf.CeilToInt(sd / 2f) + 7, BattleHelper.DamageType.Earth, 0, BattleHelper.ContactLevel.Weapon);
                    break;
                default:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.Squish);
    ...
    public override string GetHighlightText(BattleEntity caller, BattleEntity target, int level = 1)
    {
        ...
        {
            case 1:
                val = caller.DealDamageCalculation(target, sd + 1, BattleHelper.DamageType.Earth, (ulong)BattleHelper.DamageProperties.AC_Success);
                break;
            case 2:
                val = caller.DealDamageCalculation(target, sd + 4, BattleHelper.DamageType.Earth, (ulong)BattleHelper.DamageProperties.AC_Success);
                break;
            case 3:
                val = caller.DealDamageCalculation(target, sd + 7, BattleHelper.DamageType.Earth, (ulong)BattleHelper.DamageProperties.AC_Success);
                break;
            default:
                val = caller.DealDamageCalculation(target, sd - 2 + level * 3, BattleHelper.DamageType.Earth, (ulong)BattleHelper.DamageProperties.AC_Success);
                break;
        }
    ...
    */
    ...
    public override IEnumerator Execute(BattleEntity caller, int level = 1)
    {
        ...
        if (caller.curTarget != null)
        {
            ...
            {
                DealDamage(caller, sd, result, level);
            }
            else
            {
                //Miss
    ...
    public virtual void DealDamage(BattleEntity caller, int sd, bool result, int level = 1)
    {
        if (result)
        {
            ulong propertyBlock = (ulong)BattleHelper.DamageProperties.AC_Success;
...
public class LM_FlashThrow : LM_HammerThrow
{
    public LM_FlashThrow()
    {
    }
    ...
    */
    ...
    public override void DealDamage(BattleEntity caller, int sd, bool result, int level = 1)
    {
        if (result)
        {
            ulong propertyBlock = (ulong)BattleHelper.DamageProperties.AC_Success;
            switch (level)
            {
                case 2:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.ReverseSquish);
                    caller.DealDamage(caller.curTarget, sd + 3, BattleHelper.DamageType.Light, propertyBlock, BattleHelper.ContactLevel.Infinite);
                    break;
                case 1:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.ReverseSquish);
                    caller.DealDamage(caller.curTarget, sd, BattleHelper.DamageType.Light, propertyBlock, BattleHelper.ContactLevel.Infinite);
                    break;
                default:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.ReverseSquish);
                    caller.DealDamage(caller.curTarget, sd - 3 + 3 * level, BattleHelper.DamageType.Light, propertyBlock, BattleHelper.ContactLevel.Infinite);
                    break;
            }
        }
        else
        {
            switch (level)
            {
                case 2:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.ReverseSquish);
                    caller.DealDamage(caller.curTarget, Mathf.CeilToInt(sd / 2f) + 3, BattleHelper.DamageType.Light, 0, BattleHelper.ContactLevel.Infinite);
                    break;
                case 1:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.ReverseSquish);
                    caller.DealDamage(caller.curTarget, Mathf.CeilToInt(sd / 2f), BattleHelper.DamageType.Light, 0, BattleHelper.ContactLevel.Infinite);
                    break;
                default:
                    caller.curTarget.SetSpecialHurtAnim(BattleHelper.SpecialHitAnim.ReverseSquish);
                    caller.DealDamage(caller.curTarget, Mathf.CeilToInt(sd / 2f) - 3 + 3 * level, BattleHelper.DamageType.Light, 0, BattleHelper.ContactLevel.Infinite);
                    break;
            }
        }
    }
    ...
    public override string GetHighlightText(BattleEntity caller, BattleEntity target, int level = 1)
    {
        ...
        ulong propertyBlock = (ulong)BattleHelper.DamageProperties.AC_Success;
        val = caller.DealDamageCalculation(target, sd - 3 + level * 3, BattleHelper.DamageType.Light, propertyBlock);
