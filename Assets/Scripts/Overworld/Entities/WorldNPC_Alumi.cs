...
public class WorldNPC_Alumi : WorldNPCEntity
{
    ...
    public virtual void ShopInventoryInit()
    {
        ...
        {
            if ((Ribbon.RibbonType)i == Ribbon.RibbonType.DiamondRibbon)
            {
                continue;
            }

            if (pd.ribbonInventory.Find((e) => (e.type == (Ribbon.RibbonType)i)).type != (Ribbon.RibbonType)i)
            {
                ribbonPool.Add((Ribbon.RibbonType)i);
            }
        }
        List<Ribbon.RibbonType> rtList = RandomTable<Ribbon.RibbonType>.ChooseRandom(ribbonPool, 4);

        for (int i = 0; i < rtList.Count; i++)
        {
            PickupUnion pu = new PickupUnion(new Ribbon(rtList[i]));
            inventory.Add(new ShopItem(pu, Mathf.CeilToInt(1.5f * PickupUnion.GetBaseCost(pu)), Currency.Coin));
        }

        //Add diamond ribbon (?)
        if (pd.ribbonInventory.Find((e) => (e.type == Ribbon.RibbonType.DiamondRibbon)).type != Ribbon.RibbonType.DiamondRibbon)
        {
            PickupUnion pu = new PickupUnion(new Ribbon(Ribbon.RibbonType.DiamondRibbon));
            inventory.Add(new ShopItem(pu, 999, Currency.Coin));
        }
    }
    ...
    public override IEnumerator InteractCutscene()
    {
        ...
        testTextFile[13][0] = "Buy no items"
        */
        ...
        switch (state)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                if (MainManager.Instance.playerData.ribbonInventory.Count == 0)
                {
        ...
        while (state == 2)
        {
            ...
            if (itemIndexA != -1)
            {
                ribbon = MainManager.Instance.playerData.ribbonInventory[itemIndexA];
            }
            else
            {
                //cancel
            ...
            string[] tempVars = new string[] { Ribbon.GetName(ribbon), PickupUnion.GetBaseCost(new PickupUnion(ribbon)).ToString() };
            ...
            if (ynState == 1)
            {
                MainManager.Instance.playerData.RemoveRibbon(ribbon);
                MainManager.Instance.playerData.coins += PickupUnion.GetBaseCost(new PickupUnion(ribbon));
    ...
    public override string RequestTextData(string request)
    {
        ...
        if (request.Equals("sellmenu"))
        {
            ...
            {
                PickupUnion pu = new PickupUnion(pd.ribbonInventory[i]);
