...
public class WorldEnemy_FlyingChaser : WorldEnemyEntity
{
    ...
    public override void Setup()
    {
        ...
        rb.useGravity = false;  //flying = no gravity
    }
    ...
    public override void WorldUpdate()
    {
        ...
        //wandering logic
        //Allow higher y value deviation
        float distance = (Vector3.Scale(startPosition - transform.position, new Vector3(1,0.5f,1))).magnitude;
        Vector2 diff = (MainManager.XZProject(startPosition - transform.position)).normalized;

        //targets slightly above you
        //Scale makes them take an arcing path, making them easier to stomp on (or slash or smash them as they will be at your level faster)
        Vector3 diffPlayer = (Vector3.Scale(WorldPlayer.Instance.transform.position - transform.position, new Vector3(1,3,1))).normalized; //(MainManager.XZProject(WorldPlayer.Instance.transform.position - transform.position)).normalized;
        ...
        switch (state)
        {
            ...
            case ActionState.Wandering:
                ...
                newVelocity = speed * (wanderDir.x * Vector3.right + wanderDir.y * Vector3.forward) + (speed) * (yMoveB) * Vector3.up;
            ...
            case ActionState.Chasing:
                ...
                //move
                newVelocity = speed * (diffPlayer.x * Vector3.right + diffPlayer.z * Vector3.forward) + diffPlayer.y * Vector3.up;
