...
public class TestStatChangerNPCScript : WorldNPCEntity
{
    int[] raiseMaxLevels;
    int[] lowerMaxLevels;
    int[] currentLevels;

    public int costPerChange = 2;
    ...
    public override IEnumerator InteractCutscene()
    {
        ...
        testTextFile[0][0] = "Stat changer npc start<prompt,Increase stats (Y),1,Decrease stats (Y),2,Cancel,3,2>";
        testTextFile[1][0] = "Stat changer npc start<prompt,Increase stats (Y),1,Decrease stats (X),2,Cancel,3,2>";
        testTextFile[2][0] = "Stat changer npc start<prompt,Increase stats (X),1,Decrease stats (Y),2,Cancel,3,2>";
        testTextFile[3][0] = "Stat changer npc start<prompt,Increase stats (X),1,Decrease stats (X),2,Cancel,3,2>";

        testTextFile[4][0] = "Can't increase stats";
        ...
        testTextFile[14][0] = "Bye";
        testTextFile[15][0] = "Can't increase or decrease stats (too poor)";
        ...
        while (state == 1)
        {
            ...
            else
            {
                ...
                if (index == 0)
                {
                    tempVarsB = new string[] { GetCurrentHPString(), GetCurrentHPString(level) };
                }
                if (index == 1)
                {
                    tempVarsB = new string[] { GetCurrentEPString(), GetCurrentEPString(level) };
                }
                if (index == 2)
                {
                    tempVarsB = new string[] { GetCurrentSPString(), GetCurrentSPString(level) };
                }
