...
public class LauncherZone : WorldZone
{
    public float delay;
    ...
    public override void ProcessTrigger(Collider other)
    {
        ...
        if (we != null)
        {
            if (we.timeSinceLaunch > delay)
            {
                Vector3 launchVel;
                if (we.lastGroundedHeight > heightThreshold)
                {
                ...
                if (scripted)
                {
                    if (multiDirection)
                    {
                        ...
                        Vector3 oldDir = we.rb.velocity - we.rb.velocity.y * Vector3.up;
                        Vector3 newDir = oldDir.normalized * vel + launchVel.y * Vector3.up;
                        launchVel = newDir;
                    }
                    we.ScriptedLaunch(launchVel, momentumStrength);
                }
                else
                {
                    if (multiDirection)
                    ...
                    we.Launch(launchVel, momentumStrength);
                }
            }
        }
    }
